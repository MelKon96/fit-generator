...
const App = () => {
  ...
  const { user, isLoginOpen, onOpenLogin, onCloseLogin, onLogin, onLogout } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row font-sans">
      ...
      <main className="flex-1 p-4 lg:p-12">
        <Header searchQuery={searchQuery} onLogout={onLogout} user={user} onOpenLogin={onOpenLogin} setSearchQuery={setSearchQuery} filterVeggie={filterVeggie} setFilterVeggie={setFilterVeggie} filterDiabetes={filterDiabetes} setFilterDiabetes={setFilterDiabetes} userMacros={userMacros} cartCalories={cartTotals.calories} cartLength={cart.length} onOpenCart={() => setIsCartOpen(true)} sortBy={sortBy} sortOrder={sortOrder} onSort={handleSort} />
      ...
      <AuthModal isOpen={isLoginOpen} onClose={onCloseLogin} onLogin={onLogin} />
    </div>
  );
};
