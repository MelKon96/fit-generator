...
const Sidebar: React.FC<SidebarProps> = ({ onCalculate, onMagicGenerate }) => {
  return (
    <aside className="w-full lg:w-[400px] bg-white border-r border-slate-100 p-8 lg:h-screen lg:overflow-y-auto lg:sticky lg:top-0 z-20 shadow-xl lg:shadow-none">
      ...
      {/* Кнопка магической генерации */}
      <button onClick={onMagicGenerate} className="w-full mt-6 bg-gradient-to-r from-orange-500 to-amber-500 text-white py-5 rounded-[2rem] font-black uppercase tracking-widest shadow-lg hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2">
